...
namespace CSCTests
{
    ...
    public class StatisticsTests
    {
        ...
        CricketPlayer Matt = new CricketPlayer("Matt");

        private static List<string> PlayerNames = new List<string>() { "Matt", "", "", "", "", "", "", "", "", "", "" };


        CricketMatch DummyMatch = new CricketMatch("Sandon", new DateTime(2019, 5, 5), "Sandon",ResultType.Win, MatchType.League , "Matt", PlayerNames);

        CricketMatch DummyMatch2 = new CricketMatch("Aston", new DateTime(2019, 5, 5), "Aston", ResultType.Win, MatchType.League,"Matt",  PlayerNames);

        public void BattingStatsPrep_TwoMatch(int[] RS1, int[] MO1, int[] RS2, int[] MO2)
        {
            List<BattingWicketLossType> Method_Out1 = new List<BattingWicketLossType>(new BattingWicketLossType[11]);
            for (int i = 0; i < 11; ++i)
            {
                Method_Out1[i] = (BattingWicketLossType)Enum.Parse(typeof(BattingWicketLossType), MO1[i].ToString());
            }

            List<BattingWicketLossType> Method_Out2 = new List<BattingWicketLossType>(new BattingWicketLossType[11]);
            for (int i = 0; i<11;++i)
            {
                Method_Out2[i] = (BattingWicketLossType)Enum.Parse(typeof(BattingWicketLossType), MO2[i].ToString());
            }
            List<int> RunsScored1 = RS1.ToList();
            List<int> RunsScored2 = RS2.ToList();
            DummyMatch.Batting.Set_Data(RunsScored1, Method_Out1, 0);
            DummyMatch2.Batting.Set_Data(RunsScored2, Method_Out2, 0);
        ...
        { 
            ...
            DummyMatch.Bowling.SetScores(Ov1,Md1,R1,Wc1,0);
            DummyMatch2.Bowling.SetScores(Ov2, Md2, R2, Wc2,0);
        }
        ...
        public void FieldingStatsPrep_TwoMatch(int[] Ca1, int[] ROu1, int[] STu1, int[] CWi1, int[] Ca2, int[] ROu2, int[] STu2, int[] CWi2)
        {
            ...
            DummyMatch.FieldingStats.Add_Data(C1,RO1, ST1,CW1);
            DummyMatch2.FieldingStats.Add_Data(C2, RO2, ST2, CW2);
        }
        ...
        public void BattingTotalRunsTest(int[] RunsScored1, int[] MO1, int[] RunsScored2, int[] MO2, double[] expectedValue)
        {
            ...
            Assert.AreEqual(expectedValue[1], Matt.TotalInnings, "Expected Total Innings Not Same");
